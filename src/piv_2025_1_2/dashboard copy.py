...
class VIXDashboard:
    ...
    def load_data(self):
        ...
        try:
            # Cargar datos crudos
            if os.path.exists("./static/data/vix_data.csv"):
                self.df_raw = pd.read_csv("./static/data/vix_data.csv", decimal=',')
                #self.df_raw['Date'] = pd.to_datetime(self.df_raw['Date'])
                self.df_raw['Date'] = pd.to_datetime(self.df_raw['Date'], format='%Y-%m-%d', utc=True).dt.tz_convert(None)
            
            # Cargar datos enriquecidos
            if os.path.exists("./static/data/vix_data_enricher.csv"):
            ...
            if os.path.exists("./static/models/model_metadata.json"):
                import json
                with open("./static/models/model_metadata.json", 'r') as f:
